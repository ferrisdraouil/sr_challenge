...
CREATE TABLE full_years (
  id SERIAL PRIMARY KEY,
  year_id INTEGER REFERENCES seasons ON DELETE CASCADE,
  season_type TEXT REFERENCES season_types ON DELETE CASCADE; 
)
...
CREATE TABLE bye_weeks (
  ...
  season INT REFERENCES seasons ON DELETE CASCADE;
)
