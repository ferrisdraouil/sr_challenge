...
describe('updatePointsAfterBye', function() {
  ...
  test('it should correctly update additional games', async function() {
    let secondQuarterSumAfterTwoGames =
      balHomeGames[0].score.homeTeamScore.pointQ2 +
      balHomeGames[1].score.homeTeamScore.pointQ2;
    ByeWeek.updatePointsAfterBye(
      emptyPointsObj,
      'BAL',
      'home',
    ...
    expect(emptyPointsObj['BAL'].otGames).toEqual(0);
    expect(emptyPointsObj['BAL'].pointQ2).toEqual(
      secondQuarterSumAfterTwoGames
    );
  });
});
...
describe('GET /byeweek', function() {
  ...
  test('Finds the TB/MIA Week One bye', async function() {
    const tampa = await request(app).get('/byeweek/TB/2017');
    const miami = await request(app).get('/byeweek/MIA/2017');
    expect(tampa.statusCode).toBe(200);
    expect(tampa.body[0].week).toBe(1);
...
describe('GET /postbyeaverages', function() {
  ...
  test('Returns correct data when passing period', async function() {
    const philadelphia = await request(app).get(
      '/postbyeaverages/PHI/2018/first_quarter'
    );
    const carolina = await request(app).get(
      '/postbyeaverages/CAR/2018/first_quarter'
    );
    expect(philadelphia.statusCode).toBe(200);
